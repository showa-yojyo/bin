...
demo_data = r'''===== 東風戦：ランキング卓 64卓 開始 2016/01/01 00:43 =====
...
class ScoreParserState(State):
    ...
    def beginning(self, match, context, next_state):
        """Parse the beginning of a game."""

        one_game = {}
        context['games'].append(one_game)
        one_game['timestamp_start'] = match.group('timestamp')

        return context, next_state, []

    def ending(self, match, context, next_state):
        """Parse the ending of a game."""

        one_game = context['games'][-1]
        one_game['timestamp_end'] = match.group('timestamp')

        return context, next_state, []

    def ranking(self, match, context, next_state):
        ...
        return context, next_state, [
            match.group('rank'), match.group('player'), match.group('points')]
...
def main():
    ...
    input_lines = [i.strip() for i in demo_data.splitlines()]
    context = {}
